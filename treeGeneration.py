...
def treeGeneration():
    ...
    """
    summit = WeightedTree("r", randint(-5, 5))
    ...
    if chanceToHaveChildren != 1: # est-ce que nous allons ajouter des fils?
        if childrenToAdd != 0:
            ...
            for i in range(childrenToAddNow):
                summit.addChildren(WeightedTree("s", randint(-5, 5)))
            childrenToAdd -= childrenToAddNow # pour le prochain appel
            # on parcoure les fils qu'on vient d'ajouter pour en ajouter plus
            childrenToAdd = childrenToAdd // childrenToAddNow
            """ Cette ligne est
            écrite parce que ou sinon on aurait à la ligne suivante childrenToAddNow
            fois l'appel récrursif de addChildrenToTree avec chaque fois childrenToAdd
            qui est donnée on paramètre ce qui nous donnerait trop d'enfants au finale
            """
